...
(function(tvp, $) {
	...
	function reportFn(params) {
		...
		tvp.report($.extend(defaultParams, params));
	};
	...
	function endHandle() {
		...
		if ($me.config.$control && $me.config.$moreLink) {
			...
			$me.config.$qqvipSkip.find(".tvp_ads_num").text("");
	...
	function onPlaying($video, idx) {
		...
		onTimeupdateHandle = function(v) {
			...
			if ($me.adFlag != 1 && (userStatus == userStatusDef.QQVIP || userStatus == userStatusDef.QQSVIP)) {// 当前验证的信息表示用户是QQ会员，则展示qq会员跳广告文案
				...
				if (qqvipCanSkipAdCountdown > 0) {
					$me.config.$qqvipSkip.find(".tvp_ads_skip_text").addClass("tvp_disabled");
					$me.config.$qqvipSkip.find(".tvp_ads_num").text(qqvipCanSkipAdCountdown);
					$me.config.$qqvipSkip.find("._remaintime").show();
				}
				else {
					$me.config.$qqvipSkip.find("._remaintime").hide();
					$me.config.$qqvipSkip.find(".tvp_ads_skip_text").removeClass("tvp_disabled");
				}
			...
			$.each(reportObjArr, function(idx, item) {
				        ...
				        if (item && ((t > item.time && t - item.time < 1) || item.time <= t)) {
					        ...
					        tvp.report(appendParamsToUrl(item.url, params));
		...
		$me.config.$qqvipSkip.find(".tvp_ads_skip_text").off(clickEvtName).on(clickEvtName, function() {
			        $me.skipAd();
			        reportFn({
				                itype : 6,
				                val : $me.adFlag
	...
	function filterOrderList(list) {
		...
		for (idx = 0, len = list.length; idx < len; idx++) {
			...
			if (item) {// 空订单直接上报数据
				if (item.reportUrl) {
					tvp.report(appendParamsToUrl(item.reportUrl, getReportParams(item.oIdx, item)));
				}
				if (item.reportUrlOther && $.type(item.reportUrlOther.reportitem) == "string") {
					tvp.report(appendParamsToUrl(item.reportUrlOther.reportitem, {
						        url : document.URL
					        }));
				}
					...
					$.each(item.reportUrlOther.reportitem, function(idx, it) {
						        if (it && it.url) {
							        tvp.report(appendParamsToUrl(it.url, {
								                url : document.URL
							                }));
						        }
					        });
	...
	function stepReport(step) {
		...
		if (curIdx == 0 && $.isArray(orderList) && orderList[curIdx] && orderList[curIdx].reportUrl) {
			...
			tvp.report(appendParamsToUrl(orderList[curIdx].reportUrl, params));
		}
	}
	...
	tvp.Html5LoaingAd.fn = tvp.Html5LoaingAd.prototype = {
		/**
		 * 检测是否用户登录
		 */
		isLogin : function() {
			...
			$video.off("ended", onPlayAdEnd).one("ended", onPlayAdEnd).one("playing", function() {
				        if (!isPlayingAd) {
					        ...
					        if ($me.player && $me.player.config && $me.player.config.autoplay === false) {// 已经开始播放广告了，强制设置正片自动播放
						        $me.player.$video.one('tvp:video:src', function() {
							                this.load();
							                this.play();
						                });
		...
		create : function(player, config) {
			...
			if (player.$video) {
				...
				if (this.config.$copyrightTips) {
					this.config.$copyrightTips.on("touchend", ".tvp_btn_close,.tvp_ads_btn", function() {
						        $me.config.$copyrightTips.addClass(hideCls);
						        $me.player.videoTag.play();
						        $me.config.$container.addClass("tvp_ads_ontop");
					        });
				}
