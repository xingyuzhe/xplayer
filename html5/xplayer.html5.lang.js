...
(function(xplayer, $) {
	...
	xplayer.html5.lang = {
		...
		errMsg: {
			...
			"80": {
				...
				callback:function($content,errcode,data){
					if(parseInt(errcode)== 0 && xplayer.app && data && data.vid){
						var tpl = xplayer.html5skin.errorDownloader;
						xplayer.app.check(data).done(function(rs){
							if(rs.url){
								var $box = $content.find('.xplayer_player_error_content');
		...
		getErrMsg: function(errCode, errContent) {
			...
			if (errCode in xplayer.html5lang.errMsg) {
				var val = xplayer.html5lang.errMsg[errCode];
				if ($.isString(val)) return val;
				if ($.isPlainObject(val)) {
					var res = [val["main"], val["main"]?",":"", errContent in val ? (val[errContent]) : ""].join("");
					return res || xplayer.html5lang.errMsg["default"];
				}
			}
			return xplayer.html5lang.errMsg["default"];
		},
		/**
		 * 清晰度文案定义
		 * @type {Object}
		...
		getDefiName: function(key) {
			return key in xplayer.html5lang.definition ? xplayer.html5lang.definition[key] : "";
		},
		/**
		 * 根据字幕id描述获取字幕描述
		...
		getSrtName: function(id) {
			return (id in xplayer.html5lang.srtLang) ? xplayer.html5lang.srtLang[id].desc : "";
		}
	}
