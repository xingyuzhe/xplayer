...
(function(xplayer, $) {
	...
	$.extend(xplayer.Html5UI.fn, {
		buildfullscreen: function(player, $video, $control, $UILayer) {
			...
			$fullscreen = $control.find(xplayer.html5skin.elements.fullscreen);
			...
			$fullscreen.on($.os.hasTouch ? "touchend" : "click", function() {
				...
				if($.os.android && t.player.config.isHtml5UseFakeFullScreen){
					$video.removeClass('xplayer_video_with_skin');
				}				
		...
		fixClassName:function(enter){
			if(enter){
				$fullscreen.removeClass("xplayer_fullscreen").addClass("xplayer_unfullscreen");
			}
			else {
				$fullscreen.removeClass("xplayer_unfullscreen").addClass("xplayer_fullscreen");
			}
		},
		/**
		 * 判断当前是否是全屏
		...
		checkIsFullScreen: function() {
			return $fullscreen.hasClass("xplayer_unfullscreen")
		},
		/**
		 * 进入全屏
		 * @return {[type]} [description]
		...
		enterRealFullScreen: function() {
			var t = this,
				$video = t.$video,
				$fullscreen = this.$control.find(xplayer.html5skin.elements.fullscreen);
		...
		enterFakeFullScreen: function() {
			...
			this.player.$videomod.css("position", "fixed !important")
				...
				.css("width", "100%")
				.css("height", "100%").css("z-index", 1000);
			$fullscreen.removeClass('xplayer_fullscreen');
			$fullscreen.addClass('xplayer_unfullscreen');
		},
		/**
		 * 取消伪全屏API
		 * @return {[type]} [description]
		...
		cancelFakeFullScreen: function() {
			...
			$fullscreen.removeClass('xplayer_unfullscreen');
			$fullscreen.addClass('xplayer_fullscreen');
		}
	});
})(xplayer, xplayer.$);