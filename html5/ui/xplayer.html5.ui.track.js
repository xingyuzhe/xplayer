...
(function(xplayer, $) {
	...
	$.extend(xplayer.Html5UI.fn, {
		buildtrack: function(player, $video, $control, $UILayer) {
			...
			$.each(xplayer.html5skin.elements.track, function(k, v) {
				$elements[k] = $control.find(v);
			});

			$element.caption = $('<div class="xplayer_track"><span></span></div>').appendTo($UILayer),
			...
			function renderSrtListPannel(defValue) {
				...
				var defName = xplayer.html5lang.getSrtName(defValue),
					listKV = {},
					html = "";
			...
			function initTrack() {
				$video.on("xplayer:video:src", function() {
					renderSrtListPannel();
					initEvent();
					displayTrackBySflId();
